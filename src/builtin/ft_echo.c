...
/*   Updated: 2022/12/29 18:10:40 by mikim3           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int     use_u_option_check(t_simple_cmd *cmd, int *no_option_index)
{
    int index;
	...
	{
		if ((ft_strcmp_option_check(cmd->argv[index]) == 1))  // n이 맞는지 판단
...
void	ft_echo(t_tree_cmd *cmd, t_pipe *pipe_value)
{
	...
	while (cmd->argv[index])
	{
		output = ft_strjoin(output, ft_strdup(cmd->argv[index]));// strdup 헷갈리네
