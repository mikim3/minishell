...
/*   Updated: 2022/12/29 18:12:04 by mikim3           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_export(t_tree_cmd *cmd, t_list *env, t_pipe *pipe_value)
{
	int		index;
	char	*key;
	char	*value;
	...
	index = 0;
	if (cmd->argv[1] == NULL) // export 명령만 들어온경우
	{
	...
	while (cmd->argv[++index])
	{
		key = NULL;
		value = NULL;
		div_key_value(cmd->argv[index], &key, &value); //
